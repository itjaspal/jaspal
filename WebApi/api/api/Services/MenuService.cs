    ...
    public class MenuService : IMenuService
    {
        public CommonSearchView<MasterMenuView> Search(MasterMenuSearchView model)
        {
            using (var ctx = new ConXContext())
            {
                ...
                string sql = "select  menu_id, menu_name , menu_type, menu_url link_name , main_menu , icon_name from su_menu where menu_id like '%'||:p_menu_id||'%' and menu_name like '%'||:p_menu_name||'%' and menu_id  like 'MOA%' order by menu_id";
        ...
        public void Update(MasterMenuView model)
        {
            using (var ctx = new ConXContext())
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    oraCommand.CommandText = "update su_menu set menu_name = :p_menu_name , main_menu =:p_main_menu , menu_url = :p_link_name , icon_name =:p_icon_name  where menu_id = :p_menu_id";
        ...
        public MasterMenuView GetInfo(string code)
        {
            using (var ctx = new ConXContext())
            {
                string sql = "select menu_id, menu_name , menu_type, menu_url link_name , main_menu , icon_name from su_menu where menu_id = :p_menu_id";
